...
// Đánh giá 4 mức (Chỉ BGH, Admin, Tổ trưởng)
router.post(
  '/:id/evaluate',
  requireRole(Role.ADMIN, Role.HIEU_TRUONG, Role.PHO_HIEU_TRUONG, Role.TO_TRUONG),
  (req, res, next) => taskController.evaluate(req, res, next)
);

// Duyệt / Từ chối đề xuất công việc (Chỉ BGH, Admin, Tổ trưởng)
router.post(
  '/:id/approve-proposal',
  requireRole(Role.ADMIN, Role.HIEU_TRUONG, Role.PHO_HIEU_TRUONG, Role.TO_TRUONG),
  (req, res, next) => taskController.approveProposal(req, res, next)
);
router.post(
  '/:id/reject-proposal',
  requireRole(Role.ADMIN, Role.HIEU_TRUONG, Role.PHO_HIEU_TRUONG, Role.TO_TRUONG),
  (req, res, next) => taskController.rejectProposal(req, res, next)
);
